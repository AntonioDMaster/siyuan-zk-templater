...
## 模板规则
每条规则包含：
- 路径正则 *：匹配对象为 “笔记本名称/父级人类可读路径”。
  - 重要：若要匹配文件夹内的文档，请在文件夹名后直接使用 `.*`（不要加斜杠）。
  - ✅ 正确：`Workspace/Manager.*` 匹配 `/Workspace/Manager/` 下创建的文档
  - ❌ 错误：`Workspace/Manager/.*` 无法匹配（多余的斜杠破坏了模式）
- 模板 *：包含 `.md` 后缀的完整模板路径，相对于工作区根目录。示例：`data/templates/meeting.md`。
- 描述 *：规则说明。
